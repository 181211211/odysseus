...
from src.coding_agent import (
    AutonomyLevel, CodingPolicy, CodingTaskState, RepositoryInspector,
    TaskLimits, TaskStatus, assess_command, detect_test_commands,
)
...
def test_coding_task_is_registered_for_native_calls():
    assert "coding_task" in TOOL_HANDLERS
...
async def test_coding_task_persists_state(tmp_path: Path, monkeypatch):
    ...
    result = await coding_tools.CodingTaskTool().execute(json.dumps({
        "action": "start", "task_id": "calculator", "request": "create calculator tests",
        "workspace": str(tmp_path), "autonomy": "balanced",
    }), {"session_id": "test"})
