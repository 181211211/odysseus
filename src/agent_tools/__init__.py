...
TOOL_TAGS = {"bash", "python", "web_search", "web_fetch", "read_file", "write_file", "edit_file",
             ...
             "ui_control", "generate_image", "ask_user", "update_plan", "manage_tasks",
             ...
             "resolve_contact", "manage_contact", "download_model", "serve_model",
             "list_served_models", "stop_served_model", "list_downloads", "cancel_download",
             "search_hf_models", "list_cached_models", "list_serve_presets", "serve_preset",
             "adopt_served_model", "list_cookbook_servers", "edit_image", "trigger_research",
             "manage_research", "app_api"} | BUILTIN_EMAIL_TOOLS
...
# Coding-mode schemas are additive to the existing tool catalog.
FUNCTION_TOOL_SCHEMAS.extend([
    {
        "type": "function",
        "function": {
            "name": "coding_task",
            "description": "Initialize or load a bounded autonomous coding task in the active workspace. Use this at the start of a substantial repository coding request. It persists task state and enforces iteration/tool/shell/time limits.",
            "parameters": {
                "type": "object",
                "properties": {
                    "action": {"type": "string", "enum": ["start", "load", "status"]},
                    "task_id": {"type": "string"}, "request": {"type": "string"},
                    "workspace": {"type": "string"},
                    "autonomy": {"type": "string", "enum": ["safe", "balanced", "autonomous"]},
                    "max_iterations": {"type": "integer"}, "max_tool_calls": {"type": "integer"},
                    "max_shell_commands": {"type": "integer"}, "max_execution_seconds": {"type": "integer"},
                    "max_test_retries": {"type": "integer"},
                },
                "required": ["action"],
            },
        },
    },
    {
        ...
        "function": {
            "name": "coding_inspect",
            "description": "Inspect the active repository without dumping source contents. Returns bounded repository metadata, likely context files, and detected test commands. Use before broad code reading.",
            "parameters": {"type": "object", "properties": {"workspace": {"type": "string"}, "limit": {"type": "integer"}}, "required": []},
        },
    },
    {
        "type": "function",
        "function": {
            "name": "coding_git",
            "description": "Read-only Git inspection for a coding task. Supports status, diff, log, and branches. Never modifies or pushes the repository.",
            "parameters": {"type": "object", "properties": {"action": {"type": "string", "enum": ["status", "diff", "log", "branches"]}, "workspace": {"type": "string"}, "staged": {"type": "boolean"}, "limit": {"type": "integer"}}, "required": ["action"]},
        },
    },
])
